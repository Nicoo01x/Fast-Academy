...
RegisterNetEvent("sa_gangs:buyitem")
AddEventHandler("sa_gangs:buyitem",function(name,price)
  local xPlayer = ESX.GetPlayerFromId(source)
  if xPlayer then
    base = xPlayer.getAccount('bank').money
    if base >= tonumber(price) then
      -- xPlayer.removeMoney(price)
      xPlayer.removeAccountMoney('bank', price)
      xPlayer.addInventoryItem(name,1)
      TriggerClientEvent("esx:showNotification", source, "Has comprado una "..name)
    else
...
RegisterCommand("dinero", function(source)
    local xPlayer = ESX.GetPlayerFromId(source)
    xPlayer.showNotification("Tienes "..xPlayer.getAccount("bank").money.." en el banco")
end)
...
-- RegisterNetEvent("nbk_cstyle_killfeed:SyncPlayerDead")
...
--   local _src = source
--   if GetPlayerRoutingBucket(_src) == 0 then
--     if _src ~= attackerid then
--       local xPlayer = ESX.GetPlayerFromId(attackerid)
--       if xPlayer then
--           local gang = xPlayer.getGang()
--           if gang and GANGS[gang.name] then
--               local auxgang = GANGS[gang.name]
--               local kills = auxgang.getStatistics().kills
--               kills = kills + 1
--               auxgang.setStatistics('kills',kills)
--               print(gang.name..' kills:'..kills)
--               local metadata = xPlayer.getMetadata()
--               if not metadata['kills'] then
--                   metadata['kills'] = 1
--               else
--                   metadata['kills'] = metadata['kills'] + 1
--               end
--               xPlayer.setMetadata(metadata)
--           end
--       end
--     end
--   end
...
--         if gang then
--           local auxgang = gang
--           local deaths = auxgang.getStatistics().deaths
--           deaths = deaths + 1
--           auxgang.setStatistics('deaths',deaths)
--         end
--         local metadata = xPlayer.getMetadata()
--         if not metadata['deaths'] then
--             metadata['deaths'] = 1
--         else
--             metadata['deaths'] = metadata['deaths'] +1
--         end
--         xPlayer.setMetadata(metadata)
--     end
--   end
