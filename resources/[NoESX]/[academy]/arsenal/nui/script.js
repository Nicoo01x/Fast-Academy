...
function changeFilter(event) {
    ...
    if (element.textContent == 'ARMAS DE FUEGO') {
        document.querySelector('.main-title').textContent = 'ARMAS DE FUEGO'
...
async function sendData(event) {
    ...
    await fetch(`http://arsenal/${main.dataset.id}`, options)
}
...
async function sendData2(action) {
    ...
    await fetch(`http://arsenal/${action}`, options)
}
