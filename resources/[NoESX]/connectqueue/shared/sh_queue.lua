...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] fue colocado %d/%d en la cola", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("El jugador no se pudo agregar a la lista de conexión")
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("Error al agregar una identificación de prioridad, se pasaron datos no válidos")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "Fuiste bloqueado para unirte")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] se ha reincorporado a la cola después de cancelar", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] se le está cargando en el servidor")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("[Queue] Eliminado de la cola, los datos de la cola no son válidos:(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] fue eliminado de la cola porque tenían datos no válidos"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("[Queue] Eliminado debido a tiempo de espera")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] fue eliminado de la cola porque se agotó el tiempo de espera")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] se eliminó de la cola de conexión porque se agotó el tiempo de espera")
            else
                i = i + 1
            end
        end
        ...
        Queue.MaxPlayers = GetConvarInt("sv_maxclients", 129)
        Queue.Debug = GetConvar("sv_debugqueue", "true") == "true" and true or false
        Queue.DisplayQueue = GetConvar("sv_displayqueue", "true") == "true" and true or false
