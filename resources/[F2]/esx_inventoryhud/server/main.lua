...
RegisterCommand("openinventory", function(source, args, rawCommand)
    if IsPlayerAceAllowed(source, "inventory.openinventory") then
        ...
        if targetXPlayer ~= nil then
            TriggerClientEvent("esx_inventoryhud:openPlayespainrInventory", source, target, targetXPlayer.name)
        else
...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	itemShopList = {}
	local itemResult = ESX.GetItems()
	local itemInformation = {}

	for k,ir in pairs(itemResult) do
		if itemInformation[ir.name] == nil then
			itemInformation[ir.name] = {}
		end

		itemInformation[ir.name].name = ir.name
		itemInformation[ir.name].label = ir.label
		itemInformation[ir.name].limit = ir.limit
		itemInformation[ir.name].rare = ir.rare
		itemInformation[ir.name].can_remove = ir.can_remove
		itemInformation[ir.name].price = 0
		
		if shoptype == "regular" then
			for _, v in pairs(Config.Shops.RegularShop.Items) do
				if v.name == ir.name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[ir.name].name,
						label = itemInformation[ir.name].label,
						limit = itemInformation[ir.name].limit,
						rare = itemInformation[ir.name].rare,
						can_remove = itemInformation[ir.name].can_remove,
						price = v.price,
						count = 99999999
					})
				end
			...
			for _, v in pairs(Config.Shops.RobsLiquor.Items) do
				if v.name == ir.name then
					table.insert(itemShopList, {
						...
						rare = itemInformation[ir.name].rare,
						can_remove = itemInformation[ir.name].can_remove,
						price = itemInformation[ir.name].price,
						count = 99999999
					})
				end
			end
...
ESX.RegisterServerCallback("suku:getCustomShopItems", function(source, cb, shoptype, customInventory)
	...
	for i=1, #itemResult, 1 do
		...
		itemInformation[ir.name].rare = ir.rare
		itemInformation[ir.name].can_remove = ir.can_remove
		itemInformation[ir.name].price = ir.price
