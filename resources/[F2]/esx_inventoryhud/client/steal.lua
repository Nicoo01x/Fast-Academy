...
function robplayer()
	...
	if target ~= -1 and distance <= 1.5 then
		...
		if IsEntityPlayingAnim(searchPlayerPed, "random@mugging3", "handsup_standing_base", 3) or IsEntityDead(searchPlayerPed) or IsPedRagdoll(searchPlayerPed) then
			...
			Citizen.Wait(4500)
			TriggerServerEvent("hancacheadotimer", target_id)
		else
...
AddEventHandler(
	...
	function()
		...
		ESX.ShowNotification("Cacheando...")
		Citizen.Wait(3500)
		if distance < 2 then
...
function OpenBodySearchMenu(player)
	TriggerEvent("esx_inventoryhud:openPlayespainrInventory", GetPlayerServerId(player), GetPlayerName(player))
end
