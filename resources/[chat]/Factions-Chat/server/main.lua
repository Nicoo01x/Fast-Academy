...
RegisterCommand(Config.Command, function(source, args, rawCommand)
    ...
    if args ~= "" then
        ...
        elseif pJob == Jobs.Taxi then
            ...
            TriggerClientEvent('r:Message', -1, uID, uName, args, pJob)
        elseif pJob == Jobs.Sheriff then
            TriggerClientEvent('chat:addMessage', uID, { args = { "[".. Label.Sheriff .."] ".. uName .." (".. uID .."): ", "^7".. args }, color = Color.Sheriff })
            TriggerClientEvent('r:Message', -1, uID, uName, args, pJob)
...
RegisterCommand(Config.rgCommand, function(source, args, rawCommand)
    ...
    if args ~= "" then              
        ...
        if Config.JouBot then   
            TriggerEvent('JouBot:ToDiscord', 'chat', uName .. ' (ID: ' .. uID .. ')', '/' .. Config.rgCommand .. ' ' .. args, 'steam', true, uID, TTS)
        end

            if pJob == Jobs.Police or pJob == Jobs.Sheriff or pJob == Jobs.State then
                TriggerClientEvent('chat:addMessage', uID, { args = { "[".. Label.PoliceGNA .."] ".. uName .." (".. uID .."): ", "^7".. args }, color = Color.PoliceGNA })
                TriggerClientEvent('rg:Message', -1, uID, uName, args)
            else
            TriggerClientEvent('chat:addMessage', uID, { args = { "[Radio]: ", "^7No tienes permisos para hablar por este chat" }, color = Color.WeazelNews })
        end
    end
