...
Citizen.CreateThread(function()
    ...
    TriggerEvent('chat:addSuggestion', '/' .. Config.rgCommand,  'Manda un mensaje en el chat de Policía - EMS', { { name = 'Mensaje', help = 'Mensaje que quieres enviar al chat de Policía - Gendarmeria' } })
end)
...
RegisterNetEvent('rg:Message')
AddEventHandler('rg:Message', function(uID, uName, args)

    local sId = PlayerId()
    local pId = GetPlayerFromServerId(uID)
    
    if pId ~= sId then
        if PlayerData.job.name == Jobs.Police or PlayerData.job.name == Jobs.Sheriff or PlayerData.job.name == Jobs.State then
            TriggerEvent('chat:addMessage', { args = { "[".. Label.PoliceGNA .."] ".. uName .." (".. uID .."): ", "^7".. args }, color = Color.PoliceGNA })
        end
    end 
end)
