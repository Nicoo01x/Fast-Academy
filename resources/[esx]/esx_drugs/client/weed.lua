...
function ProcessWeed(xCannabis)
  ...
  TriggerServerEvent('esx_drugs:processspainCannabis')
	if(xCannabis <= 3) then
		xCannabis = 0
	end
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esx_drugs:canPickUp', function(canPickUp)
					if canPickUp then
						...
						TriggerServerEvent('esx_drugs:pickedspainUpCannabis')
					else
...
function GenerateWeedCoords()
	while true do
		...
		local coord = vector3(weedCoordX, weedCoordY, 325.23)
