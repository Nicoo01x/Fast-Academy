...
RegisterNetEvent("hancacheadotimer", function(elcacheado)
	print(elcacheado)
	Citizen.Wait(1500)
	TriggerClientEvent("bajotimer", elcacheado)
end)
...
AddEventHandler("comprobardimension", function()
    local source = source
    TriggerClientEvent("comprobardimension", source, GetPlayerRoutingBucket(source))
end)
...
RegisterCommand('PruebaCb',function(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	local gangname = xPlayer.getGang().name
	TriggerEvent('gangscript:getGangSpawnCoords', function(position)
