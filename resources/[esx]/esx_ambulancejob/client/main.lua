...
local femaleCompensationActive = false
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(5000)
    if not femaleCompensationActive and GetEntityModel(PlayerPedId()) == GetHashKey("mp_f_freemode_01") then

      if GetEntityMaxHealth(PlayerPedId()) < 200 then
        -- compensacion health female
        SetEntityMaxHealth(PlayerPedId(), 200)
        SetPedMaxHealth(PlayerPedId(), 200)
        SetEntityHealth(PlayerPedId(), 200)

        ESX.ShowNotification("Tu salud es de " .. GetEntityHealth(PlayerPedId()) .. "/" .. GetEntityMaxHealth(PlayerPedId()), 10000 )
      else
        femaleCompensationActive = true
      end
    end
  end
end)
...
AddEventHandler('esx_ambulancejob:useItem', function(itemName)
	...
	elseif itemName == 'analgesico' then
		local ped = PlayerPedId()
		ExecuteCommand('e sfallover3')
		Citizen.Wait(3000)
		ExecuteCommand('e c')
		Citizen.Wait(2000)
		TriggerEvent('esx_ambulancejob:heal', 'analgesico', true)
	elseif itemName == 'pomada' then
		local lib, anim = 'anim@heists@narcotics@funding@gang_idle', 'gang_chatting_idle01' -- TODO better animations
		local playerPed = PlayerPedId()
		...
		ESX.Streaming.RequestAnimDict(lib, function()
			...
			TriggerEvent('esx_ambulancejob:heal', 'pomada', true)
			--exports['mythic_notify']:SendAlert('inform',_U('used_bandage'))
		end)
	end
end)
...
function StartDistressSignal()
	Citizen.CreateThread(function()
		...
		while timer > 0 and isDead do
			...
			EndTextCommandDisplayText(0.4, 0.805)
...
function SendDistressSignal()
	...
	ExecuteCommand("auxilio Un ciudadano necesita asistencia médica urgente en esta ubicación!")
...
RegisterNetEvent("bajotimer", function()
	deathtimer = 5
end)

RegisterCommand("obito", function()
	if ESX.PlayerData.job.name == 'police' then
		deathtimer = 5
		ESX.ShowNotification("Obito realizado correctamente")
	else
            ...
            if GetEntityHealth(ped) <= 101 and deathtimer >= 0 then
                ...
                if not nocauteado then
                    ...
                    if deathtimer > 0 then
                        ...
                        drawTxt(" ~r~" .. deathtimer .. " ~w~ SEGUNDOS DE VIDA", 4, 0.5, 0.93, 0.50, 255, 255, 255, 255)
                    else
                        RemoveItemsAfterRPDeath()
                        isDead = false
                        nocauteado = false
                    ...
                    if hola then
                        ...
                        if ESX.PlayerData.job.name == "police" then
                            ...
                            ESX.TriggerServerCallback(
                                "guille_gangs:server:getGangsData",
                                function(gang, rank, data, boss, identifier)
                                    if gang then
                                        gangName = gang
                                        rankNum = rank
                                        gangData = data
...
local mensajeGLobal = "Has muerto"
local obj
local lootLoop = false

-- function GameEventTriggered(eventName, data)

--     if eventName == "CEventNetworkEntityDamage" then
--         victim,attacker,victimDied,weaponHash,isMeleeDamage,vehicleDamageTypeFlag = GetDataProperly(data)
...
--                 attackerid = isplayer and GetPlayerServerId(NetworkGetPlayerIndexFromPed(attacker)) or tostring(attacker==-1 and " " or attacker)
                
--                 victimName = GetPlayerName(PlayerId())
                
--                 if IsEntityAPed(attacker)  then
-- 					-- local modelHash = `prop_tool_box_05` -- The ` return the jenkins hash of a string. see more at: https://cookbook.fivem.net/2019/06/23/lua-support-for-compile-time-jenkins-hashes/
...
-- 					-- obj = CreateObject(modelHash, GetEntityCoords(PlayerPedId()), true)
-- 					-- TriggerServerEvent("glaw_mensajes:cajaLoot", GetPlayerServerId(NetworkGetPlayerIndexFromPed(victim)))
--                     -- ESX.TriggerServerCallback('glaw_mensajes:getMensajeUsuarioMuerto', function(mensaje)
...
-- 					-- 					AddTextComponentString("~o~ "..mensajeGLobal)
-- 					-- 					EndTextComponent()
...
-- 					ESX.TriggerServerCallback('glaw_mensajes:getProp', function(prop)
-- 						print(tonumber(prop))
-- 						if tonumber(prop) == 1 then
...
-- 								local part1 = StartNetworkedParticleFxNonLoopedAtCoord("scr_indep_firework_trailburst", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 								times = times - 1
-- 								Citizen.Wait(4000)
-- 							until(times == 0)
-- 						elseif tonumber(prop) == 2 then
-- 							print("prop 2 uwu")
							
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("scr_josh3") then
-- 								RequestNamedPtfxAsset("scr_josh3")
								
-- 							end
						
-- 							UseParticleFxAssetNextCall("scr_josh3")
-- 							local part2 = StartNetworkedParticleFxNonLoopedAtCoord("scr_josh3_explosion", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 3 then
...
-- 							UseParticleFxAssetNextCall("scr_indep_fireworks")
-- 							local part3 = StartNetworkedParticleFxNonLoopedAtCoord("scr_indep_firework_fountain", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 4 then
...
-- 							UseParticleFxAssetNextCall("scr_indep_fireworks")
-- 							local part4 = StartNetworkedParticleFxNonLoopedAtCoord("scr_indep_firework_burst_spawn", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 5 then
...
-- 							local part5 = StartNetworkedParticleFxNonLoopedAtCoord("scr_firework_xmas_spiral_burst_rgw", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 6 then
							
						
-- 							Wait(100)
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("wpn_musket") then
...
-- 								local part6 = StartNetworkedParticleFxNonLoopedAtCoord("muz_musket_ng", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 								times = times - 1
-- 								Citizen.Wait(1000)
-- 							until(times == 0)
-- 						elseif tonumber(prop) == 7 then
							
						
-- 							Wait(100)
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("veh_khanjali") then
...
-- 								local part7 = StartNetworkedParticleFxNonLoopedAtCoord("muz_xm_khanjali_railgun", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 								times = times - 1
-- 								Citizen.Wait(1000)
-- 							until(times == 0)
-- 						elseif tonumber(prop) == 8 then
...
-- 							repeat
-- 								UseParticleFxAssetNextCall("veh_khanjali")
-- 								local part8 = StartNetworkedParticleFxNonLoopedAtCoord("muz_xm_khanjali_railgun", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 								times = times - 1
-- 								Citizen.Wait(1000)
-- 							until(times == 0)
-- 						elseif tonumber(prop) == 9 then
							
						
-- 							Wait(100)
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("scr_xs_props") then
-- 								RequestNamedPtfxAsset("scr_xs_props")
								
-- 							end
							
-- 							UseParticleFxAssetNextCall("scr_xs_props")
-- 							local part9 = StartNetworkedParticleFxNonLoopedAtCoord("scr_xs_exp_mine_sf", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 10 then
							
						
-- 							Wait(100)
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("scr_trevor1") then
-- 								RequestNamedPtfxAsset("scr_trevor1")
								
-- 							end
							
-- 							UseParticleFxAssetNextCall("scr_trevor1")
-- 							local part10 = StartNetworkedParticleFxNonLoopedAtCoord("scr_trev1_trailer_splash", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						elseif tonumber(prop) == 11 then
							
						
-- 							Wait(100)
-- 							print(GetEntityCoords(PlayerPedId()))
-- 							if not HasNamedPtfxAssetLoaded("scr_tplaces") then
-- 								RequestNamedPtfxAsset("scr_tplaces")
								
-- 							end
							
-- 							UseParticleFxAssetNextCall("scr_tplaces")
-- 							local part10 = StartNetworkedParticleFxNonLoopedAtCoord("scr_tplaces_team_swap", GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 						end
...
-- 					-- 			PushScaleformMovieFunction(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
-- 					-- 			BeginTextComponent("STRING")
-- 					-- 			AddTextComponentString("~r~ ¡Has Muerto!")
-- 					-- 			EndTextComponent()
-- 					-- 			PopScaleformMovieFunctionVoid()
-- 					-- 			DrawScaleformMovieFullscreen(scaleform, 255, 255, 255, 255)
-- 					-- 		end
...
--                                     TriggerServerEvent('nbk_cstyle_killfeed:SyncPedDead',GetPlayerServerId(NetworkGetPlayerIndexFromPed(attacker)), weaponHash,bonehash,distance)
...
-- AddEventHandler('esx:onPlayerDeath', function(data)
-- 	ESX.TriggerServerCallback('glaw_mensajes:getProp', function(prop)
-- 		if not HasNamedPtfxAssetLoaded(Config.Efectos[prop].HasNamedPtfxAssetLoaded) then
-- 			RequestNamedPtfxAsset(Config.Efectos[prop].RequestNamedPtfxAsset)
			
-- 		end

-- 		Wait(500)
-- 		UseParticleFxAssetNextCall(Config.Efectos[prop].UseParticleFxAssetNextCall)
-- 		local part10 = StartNetworkedParticleFxNonLoopedAtCoord(Config.Efectos[prop].StartNetworkedParticleFxNonLoopedAtCoord, GetEntityCoords(PlayerPedId()), 0.0, 0.0, 0.0, 1.0, false, false, false, false)
-- 		print(part10)
-- 		RemoveNamedPtfxAsset(Config.Efectos[prop].UseParticleFxAssetNextCall) -- Clean up
...
-- local nsq = nil
-- local distanciaOBJ = nil

-- CreateThread(function()
-- 	while true do
-- 		local sleep = 1000
-- 		distanciaOBJ = #(GetEntityCoords(obj) - GetEntityCoords(PlayerPedId()))
		
-- 		--lootLoop = true
		
-- 		if distanciaOBJ < 2.0 and lootLoop then
-- 			sleep = 0
-- 			ESX.ShowFloatingHelpNotification("Presiona E para recoger el LOOT", nsq+1)
-- 		end
-- 		Citizen.Wait(sleep)
-- 	end
-- end)

-- RegisterNetEvent("glaw_mensajes:cajaLootCliente", function ()
-- 	Citizen.Wait(1000)
-- 	lootLoop = true
	
-- 	nsq = GetEntityCoords(obj)
...
function GetDataProperly(data)
    ...
    if version == 1604 then
        ...
        isMeleeDamage = tonumber(data[10]) ~= 0 and true or false
        vehicleDamageTypeFlag = tonumber(data[11])
    elseif version == 2060 then
        victim = tonumber(data[1])--1
        attacker = tonumber(data[2])--2
        victimDied = tonumber(data[5]) == 1 and true or false --5
        weaponHash = tonumber(data[6])--6
        isMeleeDamage = tonumber(data[11]) ~= 0 and true or false --11
        vehicleDamageTypeFlag = tonumber(data[12])--12
    elseif version == 2189 then
        victim = tonumber(data[1])--1
        attacker = tonumber(data[2])--2
        victimDied = tonumber(data[6]) == 1 and true or false --5
        weaponHash = tonumber(data[7])--7
        isMeleeDamage = tonumber(data[12]) ~= 0 and true or false --12
        vehicleDamageTypeFlag = tonumber(data[13])--13
    end
    return victim,attacker,victimDied,weaponHash,isMeleeDamage,vehicleDamageTypeFlag
end
...
-- CHUVA
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("lluvia",function(source,args)
